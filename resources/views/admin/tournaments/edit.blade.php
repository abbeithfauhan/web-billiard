...
@section('header')
    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Turnamen: {{ $tournament->name }}</h2>
@endsection
    ...
    <form action="{{ route('admin.tournaments.update', $tournament) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md sm:rounded-xl">
            <div class="px-4 py-5 sm:p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="entry_fee" name="entry_fee" type="number" placeholder="50000" class="mt-1 block w-full" :value="old('entry_fee', $tournament->entry_fee)" required/>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="registration_open_date" name="registration_open_date" type="datetime-local" class="mt-1 block w-full" :value="old('registration_open_date', $tournament->registration_open_date->format('Y-m-d\TH:i'))" required/>
                    ...
                    <div>
                        ...
                        <x-text-input id="registration_deadline" name="registration_deadline" type="datetime-local" class="mt-1 block w-full" :value="old('registration_deadline', $tournament->registration_deadline->format('Y-m-d\TH:i'))" required/>
                        <x-input-error class="mt-2" :messages="$errors->get('registration_deadline')" />
                    </div>
                </div>
                <div>
                    <x-input-label for="image" value="Ganti Poster/Gambar Turnamen (Opsional)" />
                    ...
                    @if($tournament->image)
                        <div class="mt-4"><p class="text-sm text-gray-500 dark:text-gray-400">Gambar saat ini:</p><img src="{{ asset('storage/' . $tournament->image) }}" alt="{{ $tournament->name }}" class="mt-2 h-40 w-auto rounded-md"></div>
                    @endif
                </div>
            </div>
            <div class="flex items-center justify-end gap-4 bg-gray-50 dark:bg-gray-700/50 px-4 py-3 text-right sm:px-6 rounded-b-xl">
                ...
                <x-primary-button>Perbarui Turnamen</x-primary-button>
            </div>
        </div>
    </form>
@endsection