...
@section('header')
    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Meja: {{ $table->name }}</h2>
@endsection
    ...
    <form action="{{ route('admin.tables.update', $table) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
            ...
            <div>
                ...
                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $table->name)" required autofocus />
            ...
            <div class="mt-4">
                ...
                <select name="type" id="type" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                    <option value="regular" {{ old('type', $table->type) == 'regular' ? 'selected' : '' }}>Regular</option>
            ...
            <div class="mt-4">
                ...
                <x-text-input id="price_per_hour" name="price_per_hour" type="number" class="mt-1 block w-full" :value="old('price_per_hour', $table->price_per_hour)" required />
            ...
            <div class="mt-4">
                ...
                <select name="is_active" id="is_active" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                    <option value="1" {{ old('is_active', $table->is_active) == 1 ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('is_active', $table->is_active) == 0 ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
            </div>
